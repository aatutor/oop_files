...
public:
	Testing(int n){
		...
		std::cout << sqr_ << std::endl;
		std::cout << pow(10, sqr_) << std::endl;
	}
};
...
class Number {
	...
	int limit_;
public:
	Number(int size): size_(size) {
		this->value_ = 0;
		// this->limit_ = pow(10, size_);
		
		this->limit_ = round( powl(10, size_) );
		// std::cout << limit_ << "\n";
		// std::cout << pow(10, size_) << "\n";
